...
export const useComparisonData = (
    ...
    const changeMap = useMemo(() => {
        console.log(
            "[useComparisonData] Calculating changeMap. Input 'result' contains change at address:",
            result.modifiedCells[0]?.address,
        );
        const map = new Map<string, ICombinedChange>();

        // Filter: Only include changes marked by user intent (not consequential recalculations)
        const userModifiedCells = result.modifiedCells.filter((change) =>
            !change.metadata?.isConsequential
        );

        for (const change of userModifiedCells) {
    ...
    const changedRowsAndCols = useMemo(() => {
        ...
        // Filter: Only include changes marked by user intent
        const userModifiedCells = result.modifiedCells.filter((change) =>
            !change.metadata?.isConsequential
        );
    ...
    const {
        ...
        console.log(
            `[useComparisonData] Calculating GridMap for sheet: ${selectedSheetName}. Received structural changes:`,
            result.structuralChanges,
        );
        // --- END OF DEBUGGING LOG ---
        const structuralChanges = result.structuralChanges.filter((c) =>
            c.sheet === selectedSheetName
        );
        const rowInsertions = structuralChanges.filter((c) =>
            c.type === "row_insertion"
        );
        const rowDeletions = structuralChanges.filter((c) =>
            c.type === "row_deletion"
        );
        const colInsertions = structuralChanges.filter((c) =>
            c.type === "column_insertion"
        );
        const colDeletions = structuralChanges.filter((c) =>
            c.type === "column_deletion"
        );

        const startSheetLogicalRows = startSheet
            ? startSheet.startRow + startSheet.data.length
            : 0;
        const endSheetLogicalRows = endSheet
            ? endSheet.startRow + endSheet.data.length
            : 0;

        const startSheetLogicalCols = startSheet
            ? startSheet.startCol + startSheet.columnCount
            : 0;
        const endSheetLogicalCols = endSheet
            ? endSheet.startCol + endSheet.columnCount
            : 0;
        ...
        const processMap = (
            ...
            while (startIdx < startLength || endIdx < endLength) {
                ...
                if (deletion) {
                    ...
                    const desc = count > 1
                        ? `Rows ${startIdx + 1}-${startIdx + count} were deleted.`
                        : `Row ${startIdx + 1} was deleted.`;
                ...
                if (insertion) {
                    ...
                    const desc = count > 1
                        ? `Rows ${endIdx + 1}-${endIdx + count} were inserted.`
                        : `Row ${endIdx + 1} was inserted.`;
                    for (let i = 0; i < count; i++) {
                        startMap.push({
                            type: "placeholder_inserted",
                            sourceIndex: -1,
                            description: desc,
                        });
                        endMap.push({
                            type: "inserted",
                            sourceIndex: endIdx + i,
                            description: desc,
                        });
                    }
                    endIdx += count;
        ...
        const { startMap: rowStartMap, endMap: rowEndMap } = processMap(
            startSheetLogicalRows,
            endSheetLogicalRows,
            rowInsertions,
            rowDeletions,
        );
        const { startMap: colStartMap, endMap: colEndMap } = processMap(
            startSheetLogicalCols,
            endSheetLogicalCols,
            colInsertions,
            colDeletions,
        );

        const startGridMap: GridMap = {
            rowMap: rowStartMap,
            colMap: colStartMap,
        };
    ...
    const unifiedColumnWidths = useMemo(() => {
        ...
        for (let i = 0; i < unifiedColCount; i++) {
            ...
            if (
                startMapItem &&
                (startMapItem.type === "original" ||
                    startMapItem.type === "deleted")
            ) {
                startWidth =
                    startSheet?.columnWidths?.[startMapItem.sourceIndex] ||
                    DEFAULT_COL_WIDTH;
            }
            if (
                endMapItem &&
                (endMapItem.type === "original" ||
                    endMapItem.type === "inserted")
            ) {
                endWidth = endSheet?.columnWidths?.[endMapItem.sourceIndex] ||
                    DEFAULT_COL_WIDTH;
            }
    ...
    const changeCoordinates = useMemo(() => {
        ...
        // Filter: Only track coordinates of changes marked by user intent
        const userModifiedCells = result.modifiedCells.filter((change) =>
            !change.metadata?.isConsequential
        );

        for (const cell of userModifiedCells) {
